...
fun List<User>.aggregate(): List<User> {
    val loginMap = this.groupBy { it.login }
    val users = mutableListOf<User>()

    loginMap.forEach { login, contributions ->
        users.add(User(login, contributions.sumOf { it.contributions }))
    }

    users.sortByDescending { it.contributions }
    return users
}